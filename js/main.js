...
let cantidadTotal;
// Cargar el JSON
const url = "https://donremoloapi.onrender.com/";
let productos = [];
...
function renderizarProds(arr) {
  arr.forEach((product) => {
    ...
    div.className =
      "card m-2 border border-start-0 border-3 d-flex flex-column align-items-center";
    div.innerHTML = `
    ...
    btn.className = "btn btn-success w-75 rounded-pill text-uppercase";
    btn.id = `btnComprar-${product._id}`;
    btn.onclick = () => agregarCarrito(product._id);
    div.appendChild(btn);
...
function agregarCarrito(id) {
  let producto = productos.find((product) => {
    ...
    return product._id == id; // agrega el prod que tiene el id que viene por param
  });

  let found = carrito.find((elemento) => elemento._id == id); //recorre el carrito y devuelve el primer elem que concide con la condicion
    ...
    pedido.innerHTML += `
            <div class="card m-3">
                ...
                <div class="card-body">
                    <h5 class="card-title text-center fw-bold">${product.name}</h5>
                    <p class="p-modal">Precio:$${product.price}</p>
                    <label class="form-label" for="${product._id}">Cant.</label>
                    <input class="form-control" id="qty-${product._id}" type="number" name="quantity" min="1" max="30" oninput="validity.valid||(value='');" value="${product.quantity}" onchange="sumQuantity('${product._id}')">
                    <div class ="d-flex flex-row-reverse justify-content-around m-4">
                    <button onclick="borrarProd('${product._id}')" type="button" class="btn btn-danger text-uppercase" id="borrar-${product._id}">Eliminar producto</button>
...
function sumQuantity(id) {
  const producto = carrito.find((product) => product._id == id);
